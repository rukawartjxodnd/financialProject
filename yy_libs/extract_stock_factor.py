...
def extractAnnualFactor(compCode):
    ...
    # 루프 돌면서 추출 (매출액, 영업이익, 당기순이익, 영업이익률(%), ROA, ROE, EPS, BPS, DPS, PER, PBR, 배당수익률 만 추출: 추후 추가 가능함)
    # 해당 항목의 index는 전체 trs에서 0, 1, 2, 13, 15, 16, 17, 18, 19, 20, 21, 23 임.
    index = 0

    for tr in trs:
        if index not in [0, 1, 2, 13, 15, 16, 17, 18, 19, 20, 21, 23]:
            index = index + 1
            continue
    ...
    return {"revenue": valuesInTrs[0], "profit": valuesInTrs[1], "income": valuesInTrs[2], "profitRatio": valuesInTrs[3],
            "roa": valuesInTrs[4], "roe": valuesInTrs[5],
            "eps": valuesInTrs[6], "bps": valuesInTrs[7], "dps": valuesInTrs[8],
            "per": valuesInTrs[9], "pbr": valuesInTrs[10],
            "divYeild": valuesInTrs[11]}


# 종목코드를 입력 받아서 해당 종목의 4개년도 대차대조표 정보 추출
# [-3년, -2년, -1년, 당해년] 순서로 리턴
def extractAnnualFinanceFactor(compCode):
    valuesInTrs = []
    inputObj = detailInfo.getDetailFinanceInfo(compCode)["annualFinanceInfoGroup"]
    trs = inputObj.find_all("tr")

    # 루프 돌면서 추출 (자산, 유동자산, 재고자산 만 추출: 추후 추가 가능함)
    # 해당 항목의 index는 전체 trs에서 0, 1, 2 임.
    index = 0
    ...
    return {"assets": valuesInTrs[0], "currentAssets": valuesInTrs[1], "inventory": valuesInTrs[2]}


# 종목코드를 입력 받아서 해당 종목의 4개년도 손익계산서 정보 추출
# [-3년, -2년, -1년, 당해년] 순서로 리턴
def extractAnnualIncomeStatFactor(compCode):
    valuesInTrs = []
    inputObj = detailInfo.getDetailFinanceInfo(compCode)["annualIncomeStatInfoGroup"]
    trs = inputObj.find_all("tr")

    # 루프 돌면서 추출 (매출액, 영업이익, 금융원가, 이자비용 만 추출: 추후 추가 가능함)
    # 해당 항목의 index는 전체 trs에서 0, 12, 24, 25 임.
    index = 0

    for tr in trs:
        if index not in [0, 12, 24, 25]:
            index = index + 1
            continue
    ...
    return {"revenue": valuesInTrs[0], "opProfit": valuesInTrs[1],
            "finCost": valuesInTrs[2], "interestCost": valuesInTrs[3]}
